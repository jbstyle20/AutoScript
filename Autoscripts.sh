...
function download_config() {
    cd
    rm -rf *
    wget https://github.com/Mr-Kenyut/AutoScript/raw/main/APN.zip >> /dev/null 2>&1
...
function dependency_install() {
    ...
    wget -q -O /etc/issue.net "https://raw.githubusercontent.com/Mr-Kenyut/AutoScript/main/ssh/issue.net" >/dev/null 2>&1
...
function install_xray() {
    # // Make Folder Xray & Import link for generating Xray
    ...
    wget -q -O /lib/systemd/system/sslh.service "${SITES}fodder/Army Phreakers Nusantara/sslh.service" >/dev/null 2>&1 
